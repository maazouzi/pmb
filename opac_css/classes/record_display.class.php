...
class record_display {
	...
	static public function get_display_coins_span($notice_id){
		...
		switch ($record_datas->get_niveau_biblio().$record_datas->get_typdoc()){
			case 'ma':// livre
				$coins_span.="book";
				$coins_span.="&amp;rft.genre=book";
				$coins_span.="&amp;rft.btitle=".rawurlencode($f?$f($record_datas->get_tit1()):$record_datas->get_tit1());
				$coins_span.="&amp;rft.title=".rawurlencode($f?$f($record_datas->get_tit1()):$record_datas->get_tit1());
			...
			case 'aa': // article
				...
				if ($parent['date']) $date_affichee = " (".$parent['date'].")";
				elseif ($parent['date_date']) $date_affichee .= " [".formatdate($parent['date_date'])."]";
				else $date_affichee="" ;
				if ($date_affichee) $coins_span.="&amp;rft.date=".rawurlencode($f?$f($date_affichee):$date_affichee);
		...
		if($record_datas->get_niveau_biblio() != "b"){
			$coins_span.="&rft_id=".rawurlencode($f?$f($opac_url_base."index.php?lvl=notice_display&id=".$record_datas->get_id()):$opac_url_base."index.php?lvl=notice_display&id=".$record_datas->get_id());	
		}else {
			$coins_span.="&rft_id=".rawurlencode($f?$f($opac_url_base."index.php?lvl=bulletin_display&id=".$parent['bulletin_id']):$opac_url_base."index.php?lvl=bulletin_display&id=".$parent['bulletin_id']);
		}
		$serie = $record_datas->get_serie();
		if($serie['name']) $coins_span.="&amp;rft.series=".rawurlencode($f?$f($serie['name']):$serie['name']);	
