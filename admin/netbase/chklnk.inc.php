					...
					if($pp->t_fields[$id_cp]['TYPE'] == "url"){
						foreach($values as $value){
							...
							if (!$response) {
								...
								echo "<div class='row'><a href=\"./cms.php?categ=editorial&sub=list\">".$o->article_title."</a>&nbsp;<a href=\"".$link."\">".$link."</a> <span class='erreur'>".$response->headers['Status-Code']." -> ".$curl->reponsecurl[$response->headers['Status-Code']]."</span></div>";
							}
						}
					}else if ($pp->t_fields[$id_cp]['TYPE'] == "resolve"){
						$options=_parser_text_no_function_("<?xml version='1.0' encoding='".$charset."'?>\n".$pp->t_fields[$id_cp]['OPTIONS'], "OPTIONS");
					...
					if($pp->t_fields[$id_cp]['TYPE'] == "url"){
						foreach($values as $value){
							...
							if (!$response) {
								...
								echo "<div class='row'><a href=\"./cms.php?categ=editorial&sub=list\">".$o->section_title."</a>&nbsp;<a href=\"".$link."\">".$link."</a> <span class='erreur'>".$response->headers['Status-Code']." -> ".$curl->reponsecurl[$response->headers['Status-Code']]."</span></div>";
							}
						}
					}else if ($pp->t_fields[$id_cp]['TYPE'] == "resolve"){
						$options=_parser_text_no_function_("<?xml version='1.0' encoding='".$charset."'?>\n".$pp->t_fields[$id_cp]['OPTIONS'], "OPTIONS");
